...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        search_mode: { type: 'string', enum: ['hybrid', 'fts', 'vector'], description: 'Optional retrieval mode. Omit for default hybrid preference with FTS fallback. Use fts for keyword-only search, hybrid for FTS + semantic fusion, or vector for semantic-only search when vectors and Ollama are available.', default: 'hybrid' },
  ...
  },
  {
    name: 'eba_get_toc',
    description:
      'Return a best-effort outline for one EBA document: cleaned section_path entries with numeric section_ref, hierarchy level, parent_section_ref, confidence, paragraph ranges, page ranges, sequence ranges, and chunk counts. Runtime cleanup filters common front-matter and consultation-response boilerplate. Use before eba_get_section when you need to understand document structure. The outline is derived from parsed headings and paragraph metadata, not a guaranteed PDF table-of-contents extraction.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
